...
def cosine_sim(v, w):
    sim = (v @ w) / (np.linalg.norm(v) * np.linalg.norm(w))
    if sim:
        return sim
    return 0
...
context_anomaly_path = 'context_anomaly_words.csv'
target_path = 'baseline_features.csv'
...
merged_df_cop = merged_df.merge(anomaly_context_df, how='inner', on='contest').copy()
#%%
context_cols = ['context_word_1', 'context_word_2', 'context_word_3']

merged_df_cop['context_words_list'] = merged_df_cop[context_cols].values.tolist()
...
print(merged_df_cop.head())
merged_df_cop.to_csv(target_path, index=False, header=False)
